    ...
    for _ in range(amount):
        ...
        draw_context.line(
            [
                start,
                (
                    start[0] + secrets.randbelow(max_length) - max_length / 2,
                    start[1] + secrets.randbelow(max_length) - max_length / 2,
                ),
            ],
            "#000000",
            width=secrets.randbelow(max_width),
        )
...
def _draw_dots(draw_context, width, height, amount=60, max_radius=8):
    for _ in range(amount):
        ...
        draw_context.ellipse(
            [
                mid[0] - secrets.randbelow(max_radius),
                mid[1] - secrets.randbelow(max_radius),
                mid[0] + secrets.randbelow(max_radius),
                mid[1] + secrets.randbelow(max_radius),
            ],
            fill="#000000",
...
def generate_captcha(
    text=None, image_width=400, image_height=200, font_height=50, vertical_change_max=80
):
    """
